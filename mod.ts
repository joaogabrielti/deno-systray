import { encodeBase64, EventEmitter, TextLineStream } from './deps.ts';

const url = Deno.cwd().replaceAll('\\', '/');
...
const getTrayPath = () => {
  ...
  switch (os) {
    case 'windows':
      path = arch === 'x86_64'
        ? `${url.replaceAll('/', '\\')}\\tray_windows_amd64.exe`
        : `${url.replaceAll('/', '\\')}\\tray_windows_386.exe`;
      break;
    case 'darwin':
      path = arch === 'x86_64'
...
export default class SysTray extends EventEmitter<Events> {
  ...
  private async run(cmd: string, ...args: string[]) {
    this._command = new Deno.Command(cmd, {
      args,
      stdin: 'piped',
      stderr: 'piped',
      stdout: 'piped',
